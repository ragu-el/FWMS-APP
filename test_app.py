...
def create_tables():
    """Create all required tables with proper column casing"""
    ...
    # Create tables with proper column casing (matching your Excel files)
    cur.execute("""
        CREATE TABLE providers (
            ...
            type VARCHAR(50),
            address TEXT,
            "City" VARCHAR(50),  -- Note the quoted capital "City"
            contact VARCHAR(50)
        );
    """)
...
def load_data():
    ...
    for table, file_path in EXCEL_FILES.items():
        ...
        # Insert data with proper column names
        with conn.cursor() as cur:
            for _, row in df.iterrows():
                # Quote column names to handle case sensitivity
                columns = ', '.join([f'"{col}"' if col == 'City' else col for col in row.index])
...
# All 15 queries with proper column casing
QUERIES = {
